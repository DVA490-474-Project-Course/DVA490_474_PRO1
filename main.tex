%===============================================================================
% !!DO NOT TOUCH!!

% LATEX TEMPLATE BY EMIL PERSSON
% Layout design: Bo Tonnquist, Baseline Management AB, 2018  
\documentclass[10pt]{projectdoc}

%===============================================================================
% QoL

\input{quality_features/packages}
\input{quality_features/acronym}
\input{quality_features/definitions}

%===============================================================================

\fancyFoot{© ProjectBase 7.0 Baseline Management AB, 2018}{\today}
% REMOVE ALL AUTOMATIC INDENTS FROM PAPER
\setlength{\parindent}{0pt}
% \usepackage[style=ieee]{biblatex}
% \addbibresource{references.bib}
\title{Project plan}
\projectname{Multi-robot Soccer - RoboCup}
\clientname{Mikael Ekström}
\managername{Mudar Ibrahim}
\begin{document}
\maketitle
\thispagestyle{fancy}
\infotable

% Copy and Paste any table into https://www.tablesgenerator.com/
% File -> From latex code... | Edit the imported table and generate new

%===============================================================================
% Sections

\input{sections/project_information}
%\input{sections/executive_summary}
%\input{sections/background}
\input{sections/purpose}
\input{sections/goal}
%\input{sections/scope}
\input{sections/limitations}
\input{sections/ethical_considerations}
\input{sections/requirements}
\input{sections/situational_analysis_and_stakeholders}
\input{sections/planning}
\input{sections/staffing}
\input{sections/project_budget}
\input{sections/development_plan}
\input{sections/communication_plan}
\input{sections/risk_analysis_and_response_planning}
\input{sections/documentation_plan}
\input{sections/testing_plan}
\input{sections/handover_plan}
\input{sections/individual_contributions}
%\input{sections/other}

\bibliographystyle{unsrtnat}
\bibliography{references}

% Should be last
\input{sections/appendix}

\end{document}
%===============================================================================